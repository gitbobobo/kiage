...
func drawTopRightControls(img *image.RGBA, rightX, y int, metric string, settingsActive bool, settingsURL string) {
	btnSz := SettingsBtnSize()
	gap := ControlsGap()
	drawExitButton(img, rightX, y)
	settingsRight := rightX - btnSz - gap
	drawSettingsButton(img, settingsRight, y, settingsActive)
	toggleRight := settingsRight - btnSz - gap
	drawMetricToggle(img, toggleRight, y, metric)
	if settingsActive && settingsURL != "" {
		drawSettingsBubble(img, settingsRight, y+btnSz+6, settingsURL)
	}
}
